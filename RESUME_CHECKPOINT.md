...
Last verified: 2026-08-30 (M16 safe operational reconciliation); M17 is next
...
- Milestones 11 through 16 are Verified. The completed ChatGPT Pro assessment is archived exactly at [`docs/reviews/2026-08-29-chatgpt-pro-product-review.md`](./docs/reviews/2026-08-29-chatgpt-pro-product-review.md), and its repository-specific synthetic-release boundary and M11–M20 delivery order are recorded in [`docs/PRODUCT_READINESS_REMEDIATION.md`](./docs/PRODUCT_READINESS_REMEDIATION.md). M12 added independently approved, time-bounded facility duties and fail-closed identity contracts. M13 added immutable protocol versions, independent authorization review, competency evidence, and exact transactional allocation gates. M14 added privacy-minimized veterinary/welfare cases with exact command, lifecycle, and audit evidence. M15 added independently approved metadata supersessions, immutable originals, exact reconciliation evidence, and corrected operational projections. M16 added atomic expected-shipment intake, independent veterinarian health decisions, census discrepancy/sign-off, capacity exceptions, and exact transfer custody reconciliation. M17 private attachments, security controls, and full synthetic recovery is next.
...
## Completed Milestone 12 Work

- Migration `0037_facility_duty_identity_assurance` adds six independent facility duties, immutable grant/revoke requests, time-bounded assignments, lifecycle history, external identity links, durable requester/decider assurance evidence, and database-enforced maker-checker/overlap rules.
- Revoking a linked identity or changing a duty increments the affected user's authorization version. Sessions, commands, and outbox reauthorization re-read current database state; stale or revoked authority fails closed.
- Duties grant only the new governed duty capabilities. They do not grant animal, cage, SOP, billing-operation, audit, or cross-lab membership authority. Existing broad role compatibility will be cut over to duty-gated workflows in the corresponding M13–M18 domains.
...
## Completed Milestone 14 Work

- Migration `0039_veterinary_welfare_cases` adds structured cases, observations, treatment orders, administration attempts, escalations, and append-only lifecycle evidence.
- Current Designated Veterinarian and Welfare Officer duties are enforced independently of base roles. Welfare Officers receive a redacted operational view; only the current veterinarian can perform treatment decisions and close cases.
- Database guards bind every mutation to the exact current command receipt, actor, authorization snapshot, lab, case, allowed transition, lifecycle event, and audit row. Missing or mismatched evidence rolls back atomically, and immutable clinical evidence cannot be rewritten or truncated.
- The guarded synthetic role-QA fixture adds one bounded welfare example. Existing health notes and quarantine records stay separate; individual clinical detail is not added to colony, directory, dashboard, or Workbook projections.
- The visible `synthetic-fail-closed-v1` marker makes the boundary explicit. Clinical terminology, formulary, service levels, retention, emergency paging, and institutional veterinary validation remain outside this repository milestone.

## Completed Milestone 15 Work

- Migration `0040_controlled_corrections` adds lab-scoped correction requests, immutable lifecycle evidence, one supersession per target, downstream reconciliation, exact command-receipt/audit parity, and guarded maker-checker decisions.
- A requester must retain the target domain's current authority. Only an independent current Data Steward with a current identity link and fresh synthetic MFA evidence can approve or reject; stale, self-decided, cross-lab, or mismatched commands fail closed.
- Safe metadata supersessions are available for litter birth, movement, lifecycle, transfer, procedure occurrence, and biosample evidence. Structural birth/weaning counts, animal status, cage endpoints, custody, procedure status, biosample identity/location/quantity, and other physical changes are blocked rather than inferred or rewritten.
- Applied masks come from immutable proposed fields. Ordinary updates to uncorrected fields remain visible, while later attempts to rewrite a corrected mutable field fail atomically. Authorized dashboard, intake, breeding, animal history, transfer, procedure, sample, API, and Workbook views display effective values with correction references; unauthorized records remain undiscoverable.
- The visible `synthetic-controlled-metadata-supersession-v1` marker makes the bounded policy explicit. This is not an institutional correction policy, regulated electronic-record claim, or authorization to alter real colony evidence.

## Completed Milestone 16 Work

- Migration `0041_safe_operational_reconciliation` adds expected shipments, resumable receipt evidence, append-only health tests and independent veterinarian decisions, census/discrepancy/capacity-exception records, dispatch-frozen custody evidence, reconciliation summaries, and exact operational lifecycle history.
- External-vendor confirmation requires an expected manifest and pre-existing empty quarantine cage. Protocol allocation, guarded animal/cage intake, quarantine admission, accepted items, summary, lifecycle, audit, and receipt commit together or roll back together. Unknown deliveries remain controlled exceptions; internal movements use transfer custody.
- Health compatibility cannot be self-declared. Any adverse/inconclusive evidence blocks, only an independent current Designated Veterinarian can decide over the exact evidence set, later evidence invalidates the decision, and quarantine release rechecks current duty, identity, evidence, and capacity.
- Census observations never auto-correct colony records. Location/outcome rules, owned discrepancies, independent sign-off, and expiring reasoned capacity exceptions are enforced at application and database boundaries.
- Transfer dispatch freezes the complete non-empty active mouse-item set. Partial/full destination receipt must match exact request items, animals, identifiers, source/destination, and custody events. Multi-parent/lab lineage and bidirectional state/event/audit/receipt parity fail closed at commit.
- This milestone is synthetic-only. Real intake, health/quarantine, veterinary, custody, and device policies remain external validation requirements.
...
- M14 clean seed and focused safety target: database/schema `mcm_test_m14_full_20260830_r19`; all 39 migrations, guarded role-QA seed with the deployment profile unset, and focused workflow/safety tests passed.
- M14 full regression target: database/schema `mcm_test_m14_full_20260830_r23`; all 14 guarded database files / 180 tests passed.
- M14 browser target: database/schema `mcm_test_m14_e2e_20260830_r24`; Desktop Chrome and Pixel 7 welfare acceptance both passed. Do not record its ephemeral local QA password.
- M15 final migration/regression target: database `mcm_test_m15_mask_20260830_r1_child`, schema `mcm_test_m15_mask_r1_child`; all 40 migrations, 17 guarded database files / 196 tests, and 13 focused live mask/projection/safety tests passed.
- M15 focused browser evidence used a fresh disposable target and passed 2/2 desktop Chromium and mobile correction workflows. Do not record its ephemeral local QA password.
- M16 final migration/regression target: database/schema `mcm_test_m16_fix4_20260830_r1`; all 41 migrations, 20 guarded database files / 218 tests, and 12 focused safety/atomicity tests passed.
- M16 parity/bootstrap evidence: schema `mcm_test_empty_verify_1788074623342_54021`; exact Prisma schema parity and empty bootstrap passed. Final forced-late migration rollback evidence used isolated `mcm_test_m16_fix_atomic_*` schemas.
- M16 focused browser evidence passed 4/4 across desktop Chromium and Pixel 7, including manager receipt, independent census sign-off, viewer/veterinarian privacy, no console/page errors, and no horizontal overflow. Do not record its ephemeral local QA password.
...
- M14 passed all 39 migrations, 121 database-free files / 592 tests, 14 guarded database files / 180 tests, the 88-entry authorization manifest, TypeScript, Prisma validation/generation, ESLint with the same five pre-existing warnings, a 54-entry production build, and diff checks. Focused Playwright passed 2/2 across Desktop Chrome and Pixel 7, including veterinarian detail, welfare-officer redaction, authoritative subject selection, no-duty denial, and horizontal-overflow checks. Independent high-risk re-review returned `SHIP` after all P0/P1 command/event/audit parity and test-isolation findings were corrected.
- M15 passed all 40 migrations, 124 database-free files / 605 tests, 17 guarded database files / 196 tests, 13 focused live projection/safety tests, the 90-entry authorization manifest, TypeScript, Prisma validation/generation, ESLint with the same five pre-existing warnings, a 55-entry production build, migration replay/schema parity/empty bootstrap, and diff checks. Focused Playwright passed 2/2 across desktop Chromium and mobile, including requester/Data Steward/viewer boundaries and responsive behavior. Independent high-risk re-review returned `SHIP` after all P0/P1 projection, evidence-pairing, test-isolation, corrected search/order/pagination, transfer-history, and stale-field-mask findings were corrected.
- M16 passed all 41 migrations, 127 database-free files / 615 tests, 20 guarded database files / 218 tests, 12 focused safety/atomicity tests, the 92-entry authorization manifest, TypeScript, Prisma validation/generation, ESLint with the same five pre-existing warnings, a 56-entry production build, migration replay/schema parity/empty bootstrap, forced late-failure rollback, and diff checks. Focused Playwright passed 4/4 across desktop Chromium and Pixel 7. Independent high-risk re-review returned `SHIP` after all P0/P1 migration, authority, parity, health-decision, census, lineage, custody, and complete dispatch-set findings were corrected.
...
3. If continuing M17 development, create a new uniquely named `mcm_test_m17_*` database and schema instead of overwriting M16 or earlier preserved evidence.
