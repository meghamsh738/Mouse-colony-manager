...
export function LabTransferRequestForm({
  ...
  const commandKey = useMemo(
    () => `${nonce}:request:${subjectType}:${sourceCageId ?? animalIds.join(",")}:${destinationLabId}:${requestedEffectiveAt}:${reason}`,
    [animalIds, destinationLabId, nonce, reason, requestedEffectiveAt, sourceCageId, subjectType],
  );
  ...
  return (
    <form action={action} className="space-y-4" data-testid="lab-transfer-request-form">
      ...
      <Button disabled={pending || reason.trim().length < 3 || !destinationLabId} type="submit">
...
function DestinationDecisionForm({ request, nonce }: { request: TransferRequestRow; nonce: string }) {
  ...
  const commandKey = `${nonce}:decide:${request.id}:${request.version}:${decision}:${destinationCageId}:${note}`;
  return (
    <form action={action} className="space-y-4">
      ...
      <FormFeedback state={state} />
      <Button disabled={pending || (request.subjectType === "animals" && decision === "accept" && !destinationCageId)} type="submit" variant={decision === "reject" ? "warning" : "default"}>
...
function RevisionForm({ request, destinationLabs, nonce }: { request: TransferRequestRow; destinationLabs: LabOption[]; nonce: string }) {
  ...
  const commandKey = `${nonce}:revise:${request.id}:${request.version}:${destinationLabId}:${requestedEffectiveAt}:${reason}`;
  return (
    <form action={action} className="space-y-4">
      <input name="transferRequestId" type="hidden" value={request.id} />
      <input name="expectedVersion" type="hidden" value={request.version} />
      ...
      <FormFeedback state={state} />
      <Button disabled={pending || reason.trim().length < 3} type="submit" variant="warning"><RefreshCw size={16} />{pending ? "Refreshing..." : "Revise packet"}</Button>
    </form>
  );
}
...
export function LabTransferWorkspace({ requests, destinationLabs, nonce }: { requests: TransferRequestRow[]; destinationLabs: LabOption[]; nonce: string }) {
        ...
        return (
          <article className="py-4" id={`transfer-${request.id}`} key={request.id}>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              {request.actions.canRevise ? <details className="border-t border-[var(--line)] pt-3"><summary className="cursor-pointer text-sm font-semibold">Revise packet</summary><div className="mt-3"><RevisionForm destinationLabs={destinationLabs} nonce={nonce} request={request} /></div></details> : null}
