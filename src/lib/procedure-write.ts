...
async function readProcedureContexts(tx: Prisma.TransactionClient, assignmentId: string, experimentId: string) {
  const [contexts] = await tx.$queryRaw<Array<{
...
async function writeProcedureAudit(
  ...
  input: {
    actorId: string;
    entityType: "procedure_plan" | "procedure_occurrence" | "experiment" | "experiment_assignment";
    entityId: string;
    action: string;
...
export async function executeCreateProcedurePlanCommand(input: {
  actor: ResolvedActor;
  ...
  if (
    !command.planId || !command.labId || !command.assignmentId || !command.sopAssignmentId || !scheduledAt
    || command.procedureCode.length < 2 || command.procedureCode.length > 80
    || command.title.length < 2 || command.title.length > 160
    || !Number.isInteger(input.expectedAssignmentVersion) || input.expectedAssignmentVersion < 1
    || !Number.isInteger(input.expectedExperimentVersion) || input.expectedExperimentVersion < 1
  ) {
    return validationError("Choose an assignment, exact assigned SOP, schedule, procedure code, and title.");
  }
  ...
  return executeIdempotentCommand({
    ...
    request: { command, expectedAssignmentVersion: input.expectedAssignmentVersion, expectedExperimentVersion: input.expectedExperimentVersion },
    ...
    handler: async (tx, context) => {
      const [assignment, sopAssignment, access] = await Promise.all([
        tx.experimentAssignment.findUnique({
          ...
          select: {
            ...
            animalId: true,
            startDate: true,
            animal: { select: { owningLabId: true, outcomeStatus: true } },
            experiment: {
              select: {
                id: true,
                labId: true,
        ...
        tx.sopAssignment.findUnique({
          ...
          select: {
            ...
            revokedAt: true,
            assignedAt: true,
            sop: { select: { active: true, currentVersionId: true, scope: true, labId: true } },
            sopVersion: {
              select: {
                versionNumber: true,
                contentHash: true,
      ...
      if (!assignment || assignment.experiment.labId !== command.labId || !canManageLab(access, command.labId)) {
        return { ok: false as const, code: "not_found", message: "Experiment assignment not found." };
      }
      if (
        assignment.version !== input.expectedAssignmentVersion
        || assignment.experiment.version !== input.expectedExperimentVersion
      ) {
        return { ok: false as const, code: "stale_conflict", message: "The assignment or experiment changed. Refresh before planning the procedure." };
      }
      if (
        !["planned", "reserved", "active"].includes(assignment.status)
        ...
        return validationError("The selected live assignment no longer accepts procedure planning.");
      }
      if (
        scheduledAt < assignment.startDate
        || (assignment.experiment.plannedStartAt && scheduledAt < assignment.experiment.plannedStartAt)
        || (assignment.experiment.plannedEndAt && scheduledAt > assignment.experiment.plannedEndAt)
      ) {
        return validationError("The procedure schedule must fall within the assignment and experiment dates.");
      }
      if (
        !sopAssignment
        ...
        || (sopAssignment.sop.scope === "lab" && sopAssignment.sop.labId !== command.labId)
        || sopAssignment.sopVersion.approval?.decision !== "approved"
      ) {
        return { ok: false as const, code: "invalid_snapshot", message: "Choose a current approved SOP assigned to this lab." };
      }
      if (
        ...
        return { ok: false as const, code: "invalid_snapshot", message: "The procedure cannot be scheduled before the SOP was created, approved, and assigned." };
      }

      const contexts = await readProcedureContexts(tx, assignment.id, assignment.experiment.id);
      if (!contexts) {
        return { ok: false as const, code: "invalid_snapshot", message: "The procedure assignment context could not be frozen for review." };
      }
      ...
      const assignmentUpdate = await tx.experimentAssignment.updateMany({
        where: { id: assignment.id, version: input.expectedAssignmentVersion, status: assignment.status },
        data: { version: { increment: 1 } },
      });
      const experimentUpdate = await tx.experiment.updateMany({
        where: { id: assignment.experiment.id, version: input.expectedExperimentVersion, status: assignment.experiment.status },
        data: { version: { increment: 1 } },
      });
      if (assignmentUpdate.count !== 1 || experimentUpdate.count !== 1) {
        return { ok: false as const, code: "stale_conflict", message: "The assignment or experiment changed while the plan was being saved." };
      }
      ...
      await Promise.all([
        ...
        writeProcedureAudit(tx, {
          ...
          newValue: { version: assignment.version + 1, procedurePlanId: plan.id },
        }),
        writeProcedureAudit(tx, {
          actorId: input.actor.id,
          ...
          newValue: { version: assignment.experiment.version + 1, procedurePlanId: plan.id },
        }),
      ]);
      return {
        ok: true as const,
...
export async function executeCancelProcedurePlanCommand(input: {
  actor: ResolvedActor;
  ...
  if (!command.planId || !command.labId || command.reason.length < 3 || command.reason.length > 400) {
  ...
  return executeIdempotentCommand({
    ...
    handler: async (tx, context) => {
      const plan = await tx.procedurePlan.findFirst({
        ...
        include: { assignment: { select: { id: true, version: true } }, experiment: { select: { id: true, version: true } } },
      });
      if (!plan || !canManageLab(await getActorLabAccess(input.actor, tx), plan.labId)) {
        return { ok: false as const, code: "not_found", message: "Procedure plan not found." };
      }
      if (plan.status !== "planned") return validationError("Only a planned procedure can be cancelled.");
      await bindProcedureCommandContext(tx, { receiptId: context.receiptId, actorId: input.actor.id, commandType: "procedure.plan.cancel" });
      const updated = await tx.procedurePlan.update({
        where: { id: plan.id },
        data: { status: "cancelled", version: { increment: 1 } },
      });
      await Promise.all([
        tx.experimentAssignment.update({ where: { id: plan.assignment.id }, data: { version: { increment: 1 } } }),
        tx.experiment.update({ where: { id: plan.experiment.id }, data: { version: { increment: 1 } } }),
        writeProcedureAudit(tx, {
          actorId: input.actor.id,
          entityType: "procedure_plan",
          entityId: plan.id,
          ...
          newValue: { status: updated.status, version: updated.version, reason: command.reason, receiptId: context.receiptId },
        }),
      ]);
      return {
        ok: true as const,
        result: { planId: plan.id, planVersion: updated.version, message: `${plan.procedureCode} cancelled.` },
        aggregateType: "procedure_plan",
        aggregateId: plan.id,
...
export async function executeRecordProcedureOccurrenceCommand(input: {
  actor: ResolvedActor;
  ...
  if (
    !command.planId || !command.labId || !command.occurrenceKey || command.occurrenceKey.length > 120 || !occurredAt
    || (command.status !== "completed" && (command.outcomeNote?.length ?? 0) < 3)
    || (command.outcomeNote?.length ?? 0) > 500
  ) {
    return validationError("Choose a valid occurrence time and describe any aborted or unperformed procedure.");
  }
  if (occurredAt.getTime() > Date.now() + 5 * 60_000) {
    return validationError("Procedure occurrence time cannot be in the future.");
  }
  ...
  return executeIdempotentCommand({
    ...
    handler: async (tx, context) => {
      const plan = await tx.procedurePlan.findFirst({
        where: { id: command.planId, labId: command.labId },
        include: {
          experiment: { select: { id: true, experimentCode: true, version: true, status: true } },
          assignment: {
            select: {
              id: true,
              version: true,
      ...
      if (!plan) return { ok: false as const, code: "not_found", message: "Procedure plan not found." };
      if (
        plan.status !== "planned"
        ...
        return validationError("The planned procedure is no longer executable for this animal assignment.");
      }
      if (
        !plan.sop.active
        ...
        return { ok: false as const, code: "invalid_snapshot", message: "The exact planned SOP is no longer current and approved for execution." };
      }
      ...
      if (!activeSopAssignment) {
        return { ok: false as const, code: "invalid_snapshot", message: "The planned SOP is no longer assigned to this lab." };
      }
      if (
        ...
        return validationError("The occurrence cannot predate its plan, assignment, or exact SOP approval and assignment.");
      }
      const currentContexts = await readProcedureContexts(tx, plan.assignmentId, plan.experimentId);
      if (
        !currentContexts
        || canonicalJsonHash(currentContexts.assignmentContext) !== canonicalJsonHash(plan.assignmentContextSnapshot)
        || canonicalJsonHash(currentContexts.experimentContext) !== canonicalJsonHash(plan.experimentContextSnapshot)
      ) {
        return { ok: false as const, code: "invalid_snapshot", message: "The assignment or operational experiment context changed after planning. Cancel and create a reviewed plan." };
      }

      await bindProcedureCommandContext(tx, { receiptId: context.receiptId, actorId: input.actor.id, commandType: "procedure.occurrence.record" });
      ...
      const occurrence = await tx.procedureOccurrence.create({
        data: {
          ...
          sopContentHash: plan.sopContentHash,
          sopAssignmentId: plan.sopAssignmentId,
          assignmentContextSnapshot: plan.assignmentContextSnapshot as Prisma.InputJsonValue,
          experimentContextSnapshot: plan.experimentContextSnapshot as Prisma.InputJsonValue,
      ...
      await Promise.all([
        ...
        writeProcedureAudit(tx, {
          ...
          action: "complete",
          previousValue: { status: plan.status, version: plan.version },
          newValue: { status: updatedPlan.status, version: updatedPlan.version, occurrenceId: occurrence.id },
        }),
      ]);
      return {
        ok: true as const,
