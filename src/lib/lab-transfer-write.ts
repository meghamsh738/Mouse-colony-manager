...
import {
  ...
  consumeProtocolReservation,
  releaseProtocolReservation,
  revalidateTransferComplianceApprovals,
...
async function releaseDestinationTransferReservation(
  ...
  if (!input.protocolAuthorizationId || !input.allocationId)
  ...
  if (!allocation)
    return {
      ...
      message: "The destination protocol reservation is missing.",
    };
  const remaining =
    allocation.reservedQuantity -
    allocation.consumedQuantity -
    allocation.releasedQuantity;
  if (remaining === 0) return { ok: true as const };
  return releaseProtocolReservation(tx, {
    actor: input.actor,
    receiptId: input.receiptId,
    protocolAuthorizationId: input.protocolAuthorizationId,
    allocationId: input.allocationId,
    quantity: remaining,
    allocationKey: `lab_transfer:${input.requestId}:${input.reasonKey}:release`,
...
async function setTransferCommandContext(
  tx: Prisma.TransactionClient,
  input: {
    requestId: string;
    actorId: string;
...
async function lockTransferRows(
  tx: Prisma.TransactionClient,
  input: {
    requestId: string;
    cageIds?: string[];
  ...
  const animalIds = [
    ...new Set((input.animalIds ?? []).filter(Boolean)),
  ].sort();
  if (animalIds.length) {
    await tx.$queryRaw(Prisma.sql`
...
async function getCurrentTransferPacket(
  tx: Prisma.TransactionClient,
  requestId: string,
) {
          ...
          include: {
            animal: {
              select: {
                ...
                currentCage: {
                  select: { id: true, barcode: true, labId: true },
                },
              },
            },
          },
  ...
  const built = buildDestinationTransferPacket({
    ...
    sourceCage: request.sourceCage
      ...
      : null,
    destinationCage:
      request.destinationCage && destinationCapacity
        ? (() => {
            const occupantSexCounts = request.destinationCage.animals.reduce(
              (counts, animal) => ({
                ...counts,
                [animal.sex]: counts[animal.sex] + 1,
              }),
    ...
    handler: async (tx, { receiptId }) => {
      if (!sourceLabId || !canRequestLabTransfer(input.actor, sourceLabId)) {
        return {
          ok: false as const,
          code: "forbidden",
          message: "A source-lab owner or manager must request this transfer.",
        };
      }
      ...
      if (
        ...
        input.command.destinationLabId === sourceLabId
      ) {
      ...
      if (!sourceLab?.active || !destinationLab?.active) {
        return transferValidationError(
          "Both source and destination labs must be active.",
        );
      }

      let sourceCageId: string | null = null;
      let animalRows: Array<{
        id: string;
      ...
      if (input.command.subjectType === "cage") {
        ...
        if (!sourceCageId)
          return transferValidationError("Choose a source cage.");
        const cage = await tx.cage.findFirst({
          ...
          select: {
            ...
            animals: {
              ...
              select: { id: true, currentCageId: true, strainId: true },
            },
          },
        });
        if (!cage || !cage.active || cage.status === "closed" || cage.closure) {
          return {
            ok: false as const,
            code: "not_found",
            message: "Active source cage not found.",
          };
        }
        if (cage.quarantineCases.length) {
          return transferValidationError(
            "Release or cancel the open quarantine case before requesting a lab transfer.",
          );
        }
        const existingCageRequest = await tx.labTransferRequest.findFirst({
          where: {
            ...
            status: {
              in: ["requested", "destination_accepted", "destination_rejected"],
            },
          },
          select: { id: true },
        });
        if (existingCageRequest)
          return transferValidationError(
            "This cage already has an open lab-transfer request.",
          );
        animalRows = cage.animals;
      } else {
        const animalIds = [
          ...new Set(
            (input.command.animalIds ?? [])
              .map((id) => id.trim())
              .filter(Boolean),
          ),
        ];
        if (!animalIds.length || animalIds.length > 50) {
          return transferValidationError(
            "Choose between 1 and 50 animals for this transfer.",
          );
        }
        ...
        if (animals.length !== animalIds.length) {
          return {
            ...
            code: "not_found",
            message:
              "One or more source-lab animals were not found or are no longer transferable.",
          };
        }
        if (
          animals.some((animal) => animal.currentCage?.quarantineCases.length)
        ) {
          return transferValidationError(
            "Release or cancel open quarantine cases before requesting a lab transfer.",
          );
        }
        animalRows = animals.map(({ id, currentCageId, strainId }) => ({
          id,
          currentCageId,
          strainId,
        }));
      ...
      if (animalRows.length) {
        ...
        if (existingItem)
          return transferValidationError(
            "One or more animals already belong to an open lab-transfer request.",
          );
      }
      ...
      const compliance = await evaluateAndWriteComplianceEvidence(tx, {
        ...
        protocolAuthorizationId: input.command.sourceProtocolAuthorizationId,
        strainIds: animalRows.map((animal) => animal.strainId),
        procedureCode: "transfer",
        requiredPersonnelRoles: ["transfer_coordinator"],
        countOperation: "none",
      });
      ...
      await tx.labTransferRequest.create({
        data: {
          ...
          sourceProtocolAuthorizationId: compliance.protocolAuthorizationId,
          sourceComplianceEvidenceSnapshotId: compliance.evidenceSnapshotId,
        },
      });
      if (animalRows.length) {
      ...
      await tx.labTransferPacket.create({
        data: {
          ...
          version: 1,
          destinationPayload:
            current.packet as unknown as Prisma.InputJsonValue,
      ...
      return {
        ...
        result: {
          requestId,
          message: "Transfer request sent to the destination lab.",
        },
        aggregateType: "lab_transfer_request",
        aggregateId: requestId,
        resultingVersion: 1,
...
export async function executeDecideLabTransferCommand(input: {
  ...
  return executeIdempotentCommand({
    ...
    handler: async (tx, { receiptId }) => {
      const request = await tx.labTransferRequest.findUnique({
        ...
        include: {
          ...
          items: {
            ...
            select: {
              animalId: true,
              animal: { select: { id: true, sex: true, strainId: true } },
            },
          },
        },
      });
      if (
        !request ||
        !canDecideLabTransfer(input.actor, request.destinationLabId)
      ) {
        return {
          ok: false as const,
          code: "not_found",
      ...
      if (request.status !== "requested") {
        return {
          ...
          message: "This request is not waiting for a destination decision.",
        };
      }
      ...
      if (
        !latestPacket ||
        !current ||
        latestPacket.version !== request.packetVersion ||
        latestPacket.payloadHash !== current.payloadHash
      ) {
        return {
          ok: false as const,
          code: "packet_stale",
          message:
            "The transfer details changed. Ask the source lab to refresh the packet before deciding.",
        };
      }
      ...
      if (input.command.decision === "reject") {
        if (!note || note.length < 3)
          return transferValidationError(
            "Enter a reason for rejecting this transfer.",
          );
      ...
      return withComplianceWriteScope(
        tx,
        {
          ...
          protocolAuthorizationId:
            input.command.destinationProtocolAuthorizationId,
          strainIds: request.items.map((item) => item.animal.strainId),
          procedureCode: "transfer",
          requiredPersonnelRoles: ["transfer_coordinator"],
          countOperation: "reserve",
          quantity: request.items.length,
          allocationKey: `lab_transfer:${request.id}:destination:packet:${request.packetVersion}`,
        },
        async (destinationCompliance) => {
          ...
          if (request.subjectType === "animals") {
            ...
            if (!destinationCageId)
              return transferValidationError(
                "Choose an active destination cage.",
              );
            const [cage, mixedSexRule] = await Promise.all([
              tx.cage.findFirst({
                where: {
                  id: destinationCageId,
                  labId: request.destinationLabId,
                },
                select: {
                  id: true,
            ...
            if (
              !cage ||
              !cage.active ||
              ["closed", "retired"].includes(cage.status) ||
              cage.closure ||
              cage.quarantineCases.length
            ) {
              return transferValidationError(
                "Choose an active destination cage without an open quarantine case.",
              );
            }
            ...
            if (quarantineError)
              return transferValidationError(quarantineError);
            ...
            if (request.items.length > capacity.remainingCapacity) {
              return transferValidationError(
                `That cage has ${capacity.remainingCapacity} spaces for ${request.items.length} animals.`,
              );
            }
            packetVersion += 1;
            ...
            const withDestination = await getCurrentTransferPacket(
              tx,
              request.id,
            );
            if (!withDestination)
              throw new Error(
                "Destination transfer packet could not be built.",
              );
            ...
            await tx.labTransferPacket.create({
              data: {
                ...
                destinationPayload:
                  withDestination.packet as unknown as Prisma.InputJsonValue,
          ...
          const updated = await tx.labTransferRequest.update({
            ...
            data: {
              ...
              destinationDecisionNote: note,
              destinationProtocolAuthorizationId:
                destinationCompliance.protocolAuthorizationId,
              destinationComplianceEvidenceSnapshotId:
                destinationCompliance.evidenceSnapshotId,
              destinationProtocolCountAllocationId:
                destinationCompliance.allocationId,
              version: { increment: 1 },
            },
            select: { version: true },
          });
          ...
          return {
            ...
            result: {
              requestId: request.id,
              message: "Transfer accepted. CMU finalization is now required.",
            },
            aggregateType: "lab_transfer_request",
            aggregateId: request.id,
            resultingVersion: updated.version,
...
export async function executeReviseLabTransferCommand(input: {
  ...
  return executeIdempotentCommand({
    ...
    handler: async (tx, { receiptId }) => {
      ...
      if (
        !request ||
        !canRequestLabTransfer(input.actor, request.sourceLabId)
      ) {
        return {
          ok: false as const,
          code: "not_found",
      ...
      if (
        !input.command.destinationLabId ||
        input.command.destinationLabId === request.sourceLabId
      ) {
        return transferValidationError("Choose a different destination lab.");
      }
      ...
      if (!destinationLab?.active)
        return transferValidationError("Choose an active destination lab.");

      await lockTransferRows(tx, {
        requestId: request.id,
        cageIds: [
          request.sourceCageId ?? "",
          ...request.items.map((item) => item.sourceCageId ?? ""),
        ],
        animalIds: request.items.map((item) => item.animalId),
      });

      let animalRows = request.items.map((item) => ({
        id: item.animalId,
        currentCageId: item.sourceCageId,
        strainId: item.animal.strainId,
      }));
      if (request.subjectType === "cage") {
        ...
        if (
          ...
          return transferValidationError(
            "The source cage is no longer transferable.",
          );
        }
        animalRows = cage.animals;
      } else {
        ...
        if (
          animals.length !== request.items.length ||
          animals.some((animal) => animal.currentCage?.quarantineCases.length)
        ) {
          return transferValidationError(
            "One or more animals are no longer transferable.",
          );
        }
        animalRows = animals.map(({ id, currentCageId, strainId }) => ({
          id,
      ...
      if (animalRows.length) {
        ...
        if (conflict)
          return transferValidationError(
            "One or more animals now belong to another open transfer request.",
          );
      }

      return withM13MutationSavepoint(tx, async () => {
        const released = await releaseDestinationTransferReservation(tx, {
          requestId: request.id,
          protocolAuthorizationId: request.destinationProtocolAuthorizationId,
          allocationId: request.destinationProtocolCountAllocationId,
          actor: input.actor,
          receiptId,
          reasonKey: "revise",
        });
        if (!released.ok) return released;
        const sourceCompliance = await evaluateAndWriteComplianceEvidence(tx, {
          actor: input.actor,
          receiptId,
          commandType: "lab_transfer.revise",
          ...
          aggregateId: request.id,
          protocolAuthorizationId:
            input.command.sourceProtocolAuthorizationId ??
            request.sourceProtocolAuthorizationId,
          strainIds: animalRows.map((animal) => animal.strainId),
          procedureCode: "transfer",
          requiredPersonnelRoles: ["transfer_coordinator"],
          countOperation: "none",
        });
        if (!sourceCompliance.ok) return sourceCompliance;

        await tx.labTransferItem.deleteMany({
        ...
        const updated = await tx.labTransferRequest.update({
          ...
          data: {
            ...
            sourceProtocolAuthorizationId:
              sourceCompliance.protocolAuthorizationId,
            sourceComplianceEvidenceSnapshotId:
              sourceCompliance.evidenceSnapshotId,
            destinationProtocolAuthorizationId: null,
            destinationComplianceEvidenceSnapshotId: null,
            destinationProtocolCountAllocationId: null,
            version: { increment: 1 },
          },
          select: { version: true },
        });
        const current = await getCurrentTransferPacket(tx, request.id);
        if (!current)
          throw new Error("Revised transfer packet could not be built.");
        await tx.labTransferPacket.create({
          data: {
            id: randomUUID(),
            requestId: request.id,
        ...
        return {
          ...
          result: {
            requestId: request.id,
            message:
              "Transfer packet revised. Destination approval is required again.",
          },
          aggregateType: "lab_transfer_request",
          aggregateId: request.id,
          resultingVersion: updated.version,
...
export async function executeCancelLabTransferCommand(input: {
  ...
  return executeIdempotentCommand({
    ...
    handler: async (tx, { receiptId }) => {
      ...
      if (reason.length < 3)
        return transferValidationError(
          "Enter a reason for cancelling this transfer.",
        );
      return withM13MutationSavepoint(tx, async () => {
        const released = await releaseDestinationTransferReservation(tx, {
          requestId: request.id,
        ...
        await tx.labTransferItem.updateMany({
...
async function getTransferBlockers(
  tx: Prisma.TransactionClient,
  animalIds: string[],
) {
  if (!animalIds.length) {
  ...
  const [
    ...
    tx.experimentAssignment.findMany({
      ...
      select: {
        ...
        endDate: true,
        version: true,
        complianceEvidenceSnapshotId: true,
...
async function applyTransferOverride(
  tx: Prisma.TransactionClient,
  input: {
    actor: ResolvedActor;
    requestId: string;
  ...
  for (const assignment of input.blockers.experimentAssignments) {
    ...
    if (
      ...
      return {
        ...
        code: "compliance_count_conflict",
        message:
          "An active experiment assignment has no exact protocol reservation. Reconcile the legacy assignment before overriding this transfer.",
      };
    }
    ...
    if (
      ...
      return {
        ...
        code: "compliance_count_conflict",
        message:
          "An experiment assignment has an inconsistent protocol reservation. Reconcile it before overriding this transfer.",
      };
    }
  }
  for (const assignment of input.blockers.experimentAssignments) {
    ...
    const released = await releaseProtocolReservation(tx, {
      actor: input.actor,
      receiptId: input.commandReceiptId,
      protocolAuthorizationId: assignment.experiment.protocolAuthorizationId,
      allocationId: assignment.protocolCountAllocationId!,
      quantity: remaining,
      allocationKey: `lab_transfer:${input.requestId}:source_assignment:${assignment.id}:release`,
      aggregateType: "lab_transfer_request",
      aggregateId: input.requestId,
    });
    if (!released.ok) return released;
  ...
  const experiments = new Map<
    string,
    { version: number; assignmentIds: string[] }
  >();
  for (const assignment of input.blockers.experimentAssignments) {
  ...
  for (const [experimentId, experiment] of experiments) {
    ...
    if (updated.count !== 1) {
      throw new Error(
        "An experiment changed while applying the transfer override. Refresh and review the request.",
      );
    }
  ...
  for (const assignment of input.blockers.experimentAssignments) {
    const endDate =
      assignment.startDate > input.movedAt
    ...
    if (updated.count !== 1) {
      throw new Error(
        "An experiment assignment changed while applying the transfer override. Refresh and review the request.",
      );
    }
    await tx.auditLog.create({
      data: {
  ...
  for (const allocation of input.blockers.projectAllocations) {
    await tx.animalProjectAllocation.update({
      ...
      data: {
        endedAt:
          allocation.startedAt > input.movedAt
    ...
    await tx.auditLog.create({
      data: {
        ...
        newValue: {
          endedAt: (allocation.startedAt > input.movedAt
            ? allocation.startedAt
            : input.movedAt
          ).toISOString(),
          requestId: input.requestId,
  ...
  const setupById = new Map(
    input.blockers.breedingAdults.map((adult) => [
      adult.breedingSetup.id,
      adult.breedingSetup,
    ]),
  );
  for (const setup of setupById.values()) {
    const transitioned = await transitionBreedingSetup(
      {
        ...
        happenedAt: (setup.startDate > input.movedAt
          ? setup.startDate
          : input.movedAt
        ).toISOString(),
    ...
    if (!transitioned.ok) {
      throw new Error(
        `Breeding setup ${setup.id} could not be retired: ${transitioned.message}`,
      );
    }
  }
  await createTransferEvent(tx, {
    ...
    reason: input.reason,
    metadata: {
      experimentAssignmentIds: input.blockers.experimentAssignments.map(
        (assignment) => assignment.id,
      ),
      projectAllocationIds: input.blockers.projectAllocations.map(
    ...
    handler: async (tx, { receiptId }) => {
      if (!canFinalizeLabTransfer(input.actor)) {
        return {
          ok: false as const,
          code: "forbidden",
          message: "CMU or Facility finalization authority is required.",
        };
      }
      const initial = await tx.labTransferRequest.findUnique({
        ...
        include: {
          items: {
            where: { active: true },
            select: { animalId: true, sourceCageId: true },
          },
          packets: { orderBy: { version: "desc" }, take: 1 },
        },
      });
      ...
      await lockTransferRows(tx, {
        ...
        cageIds: [
          initial.sourceCageId ?? "",
          initial.destinationCageId ?? "",
          ...initial.items.map((item) => item.sourceCageId ?? ""),
        ],
      ...
      if (
        ...
        return {
          ...
          message:
            "The destination lab must accept the current packet before finalization.",
        };
      }
      if (
        latestPacket.version !== request.packetVersion ||
        current.payloadHash !== latestPacket.payloadHash
      ) {
        return {
          ok: false as const,
          code: "packet_stale",
          message:
            "Operational details changed after approval. Refresh the packet and obtain destination approval again.",
        };
      }
      if (dateKey(request.requestedEffectiveAt) > dateKey(new Date())) {
        return transferValidationError(
          `This transfer is scheduled for ${dateKey(request.requestedEffectiveAt)}.`,
        );
      }
      if (!request.sourceLab.active || !request.destinationLab.active) {
        return transferValidationError(
          "Both source and destination labs must remain active.",
        );
      }
      const compliance = await revalidateTransferComplianceApprovals(tx, {
        requestId: request.id,
        sourceLabId: request.sourceLabId,
        destinationLabId: request.destinationLabId,
        sourceProtocolAuthorizationId: request.sourceProtocolAuthorizationId,
        sourceEvidenceSnapshotId: request.sourceComplianceEvidenceSnapshotId,
        destinationProtocolAuthorizationId:
          request.destinationProtocolAuthorizationId,
        destinationEvidenceSnapshotId:
          request.destinationComplianceEvidenceSnapshotId,
        destinationAllocationId: request.destinationProtocolCountAllocationId,
        itemCount: request.items.length,
        strainIds: request.items.map((item) => item.animal.strainId),
        finalizer: input.actor,
      });
      if (!compliance.ok) return compliance;
      await setTransferCommandContext(tx, {
        requestId: request.id,
      ...
      if (request.subjectType === "cage") {
        ...
        if (
          !sourceCage ||
          sourceCage.labId !== request.sourceLabId ||
          !sourceCage.active ||
          sourceCage.status === "closed" ||
          sourceCage.closure
        ) {
          return transferValidationError(
            "The source cage is no longer transferable.",
        ...
        if (sourceCage.quarantineCases.length) {
          return transferValidationError(
            "Release or cancel the open quarantine case before finalization.",
          );
        }
        const currentIds = sourceCage.animals.map((animal) => animal.id).sort();
        const approvedIds = [...animalIds].sort();
        if (
          JSON.stringify(currentIds) !== JSON.stringify(approvedIds) ||
          sourceCage.animals.some(
            (animal) => animal.owningLabId !== request.sourceLabId,
          )
        ) {
          return {
            ok: false as const,
            code: "packet_stale",
            message:
              "The cage occupants changed. Refresh the packet and obtain destination approval again.",
          };
        }
      } else {
        if (!request.destinationCageId)
          return transferValidationError(
            "The accepted packet has no destination cage.",
          );
        const [approvedDestination, mixedSexRule] = await Promise.all([
          tx.cage.findUnique({
            ...
            select: {
              ...
              closure: { select: { id: true } },
              room: {
                select: { facility: { select: { maxCageOccupancy: true } } },
              },
              animals: {
                where: { outcomeStatus: "alive" },
                select: { id: true, sex: true },
        ...
        if (
          !destinationCage ||
          destinationCage.labId !== request.destinationLabId ||
          !destinationCage.active ||
          ["closed", "retired"].includes(destinationCage.status) ||
          destinationCage.closure ||
          destinationCage.quarantineCases.length
        ) {
          return transferValidationError(
            "The approved destination cage is no longer available.",
          );
        }
        ...
        if (animalIds.length > capacity.remainingCapacity) {
          return transferValidationError(
            `The approved destination cage now has only ${capacity.remainingCapacity} spaces.`,
          );
        }
        const animals = await tx.animal.findMany({
        ...
        if (
          ...
          animals.some(
            (animal) =>
              ...
              TERMINAL_ANIMAL_STATUSES.includes(
                animal.status as (typeof TERMINAL_ANIMAL_STATUSES)[number],
              ) ||
              animal.currentCage?.quarantineCases.length,
          )
        ) {
          return transferValidationError(
            "One or more approved animals are no longer transferable.",
          );
        }
      }

      return withM13MutationSavepoint(tx, async () => {
        if (!request.destinationProtocolCountAllocationId) {
          return {
            ok: false as const,
            code: "compliance_count_conflict",
            message:
              "The accepted destination protocol reservation is missing.",
          };
        }
        const consumed = await consumeProtocolReservation(tx, {
          actor: input.actor,
          receiptId,
          protocolAuthorizationId: request.destinationProtocolAuthorizationId,
          allocationId: request.destinationProtocolCountAllocationId,
          allocationKey: `lab_transfer:${request.id}:finalize:consume`,
          aggregateType: "lab_transfer_request",
          aggregateId: request.id,
        });
        ...
        const plannedProcedureBlockers = blockers.blockers.filter(
          (blocker) => blocker.kind === "procedure_plan",
        );
        if (plannedProcedureBlockers.length) {
          return {
            ...
            message:
              "Cancel each planned procedure before transferring the animal to another lab.",
            result: {
              blockers: plannedProcedureBlockers,
            } as unknown as Prisma.InputJsonValue,
          };
        }
        if (blockers.blockers.length && !overrideReason) {
          return {
            ...
            code: "active_relationships",
            message:
              "Active experiment, project, or breeding relationships block this transfer.",
            result: {
        ...
        if (blockers.blockers.length) {
          if (!canOverrideLabTransferBlocks(input.actor)) {
            return {
              ...
              message:
                "Only the Facility Admin can override active research relationships.",
            };
          }
          if (!overrideReason || overrideReason.length < 10) {
            return transferValidationError(
              "Enter a detailed Facility override reason of at least 10 characters.",
            );
          }
          const overridden = await applyTransferOverride(tx, {
            actor: input.actor,
            requestId: request.id,
          ...
          return transferValidationError(
            "No active relationship override is required for this transfer.",
          );
        }

        if (request.subjectType === "cage" && sourceCage) {
          const categoryId =
            sourceCage.chargePeriods[0]?.categoryId ??
            (await getDefaultChargeCategoryId(tx));
          if (!categoryId)
            return transferValidationError(
              "Create an active cage charge category before finalization.",
            );
          if (
            sourceCage.chargePeriods[0]?.startedAt &&
            sourceCage.chargePeriods[0].startedAt > movedAt
          ) {
            return transferValidationError(
              "The active cage charge period starts after the transfer time.",
            );
          }
          ...
          if (animalIds.length) {
            ...
            if (updatedAnimals.count !== animalIds.length)
              throw new Error("Transfer subjects changed during finalization.");
        ...
        await resolveNotificationsForOwnershipChangeInTransaction(tx, {
          sourceLabId: request.sourceLabId,
          cageIds:
            request.subjectType === "cage" && sourceCage ? [sourceCage.id] : [],
          animalIds,
          resolvedAt: movedAt,
        });
        ...
        const finalized = await tx.labTransferRequest.update({
          ...
          data: {
            ...
            finalizedAt: movedAt,
            overrideApprovedById: blockers.blockers.length
              ? input.actor.id
              : null,
        ...
        return {
          ...
          result: {
            requestId: request.id,
            message:
              "Transfer finalized. Ownership and billing are now updated.",
          },
          aggregateType: "lab_transfer_request",
          aggregateId: request.id,
