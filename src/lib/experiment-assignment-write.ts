...
const planningAnimalStatuses = ["colony_holding", "reserved", "experiment_completed"] as const;
...
function hasValidIdentity(input: CommandIdentity) {
  return input.idempotencyKey.trim().length > 0 && input.requestId.trim().length > 0;
}
...
function dateFallsWithinExperiment(
  ...
  return (!experiment.plannedStartAt || value >= dateKey(experiment.plannedStartAt))
    && (!experiment.plannedEndAt || value <= dateKey(experiment.plannedEndAt));
}

function assignmentDatesAreValid(assignment: { startDate: Date; endDate: Date | null }) {
  return !Number.isNaN(assignment.startDate.valueOf())
    && (!assignment.endDate || (
...
function assignmentStaleConflict(assignmentId: string, expectedVersion: number, currentVersion: number | null) {
  return {
    ok: false as const,
    code: "stale_conflict",
    message: "An experiment assignment changed after you opened it. Refresh and review the cohort before trying again.",
    result: {
...
export async function executeReserveAnimalForExperimentCommand(input: {
  actor: ResolvedActor;
  command: {
    animalId: string;
    experimentId: string;
  ...
  if (
    ...
    return validationError("Submit the exact animal and experiment snapshot with valid versions and a calendar date.");
  }
    ...
    handler: async (tx, context) => {
      const experiment = await tx.experiment.findFirst({
        where: {
          ...
          ...(input.actor.canonicalRole === "lab_user" ? { labId: input.actor.activeLabId ?? "__none__" } : {}),
        },
        select: {
          id: true,
          labId: true,
      ...
      if (!experiment || !mutableExperimentStatuses.includes(experiment.status)) {
        return { ok: false, code: "not_found", message: "The experiment was not found or no longer accepts reservations." };
      }
      if (experiment.version !== input.expectedExperimentVersion) {
        return staleConflict("experiment", experiment.id, input.expectedExperimentVersion, experiment.version);
      }
      if (experiment.project.labId !== experiment.labId) {
        return invalidSnapshot("The experiment project is no longer valid for the experiment lab.");
      }
      if (!dateFallsWithinExperiment(startDate.date, experiment)) {
        return invalidSnapshot("The reservation date is outside the experiment's current planned date range.");
      }
      ...
      if (!animal) {
        return { ok: false, code: "not_found", message: "The animal was not found." };
      }
      if (animal.version !== input.expectedAnimalVersion) {
        return staleConflict("animal", animal.id, input.expectedAnimalVersion, animal.version);
      }
      if (animal.owningLabId !== experiment.labId) {
        return invalidSnapshot("The animal and experiment must belong to the same lab.");
      }
      if (animal.outcomeStatus !== "alive" || animal.status !== "colony_holding") {
        return invalidSnapshot(`${animal.animalId} is no longer live and available in colony holding.`);
      }
      if (!animal.alleles.length || animal.alleles.some((allele) => allele.callStatus !== "confirmed")) {
        return invalidSnapshot(`${animal.animalId} requires confirmed genotype calls before reservation.`);
      }
      if (animal.experimentAssignments.some((assignment) => assignment.experimentId === experiment.id)) {
        return { ok: false, code: "duplicate_assignment", message: `${animal.animalId} is already linked to this experiment.` };
      }
      if (animal.experimentAssignments.some((assignment) => assignment.status === "reserved" || assignment.status === "active")) {
        return { ok: false, code: "reservation_conflict", message: `${animal.animalId} already has an active or reserved experiment assignment.` };
      }

      const assignmentId = randomUUID();
      const timestamp = new Date();
      const mutation = await tx.$queryRaw<Array<{
        assignmentId: string;
        assignmentVersion: number;
        animalVersion: number;
        experimentVersion: number;
        ...
        WITH eligible_experiment AS MATERIALIZED (
          ...
          SELECT
            ...
            ${command.treatmentGroup}, ${command.notes}, TRUE, 1
      ...
      if (!committed) {
        const [currentAnimal, currentExperiment] = await Promise.all([
          tx.animal.findUnique({ where: { id: animal.id }, select: { version: true } }),
          tx.experiment.findUnique({ where: { id: experiment.id }, select: { version: true } }),
        ]);
        if (currentAnimal?.version !== input.expectedAnimalVersion) {
          return staleConflict("animal", animal.id, input.expectedAnimalVersion, currentAnimal?.version ?? null);
        }
        if (currentExperiment?.version !== input.expectedExperimentVersion) {
          return staleConflict("experiment", experiment.id, input.expectedExperimentVersion, currentExperiment?.version ?? null);
        }
        return {
          ok: false,
          code: "reservation_conflict",
          message: "The animal or experiment changed while the reservation was being committed. Refresh before trying again.",
        };
      }
      ...
      await writeAudit(tx, {
        ...
        newValue: {
          ...
          autoAllocationMarker: createdAllocationId ? experimentReservationAllocationMarker(assignmentId) : null,
        },
      });
      await writeAudit(tx, {
        actorId: input.actor.id,
...
export async function executePlanExperimentAssignmentsCommand(input: {
  actor: ResolvedActor;
  command: {
    experimentId: string;
    startDate: string;
  ...
  if (
    ...
    || assignments.some((assignment) => !assignment.animalId || !assignment.treatmentGroup || assignment.treatmentGroup.length > 80)
    || !hasUniqueValues(assignments.map((assignment) => assignment.animalId))
    || (notes?.length ?? 0) > 400
  ) {
    return validationError("Submit a valid, duplicate-free rendered cohort snapshot and refresh the experiment version.");
  }
  ...
  return executeIdempotentCommand<AssignmentCommandResult>({
    ...
    handler: async (tx, context) => {
      ...
      if (!experiment || !mutableExperimentStatuses.includes(experiment.status)) {
        return { ok: false, code: "not_found", message: "The experiment was not found or no longer accepts cohort planning." };
      }
      if (input.actor.canonicalRole === "lab_user" && experiment.labId !== input.actor.activeLabId) {
        return { ok: false, code: "forbidden", message: "The experiment is outside your active lab." };
      }
      if (experiment.version !== input.expectedExperimentVersion) {
        return staleConflict("experiment", experiment.id, input.expectedExperimentVersion, experiment.version);
      }
      if (!dateFallsWithinExperiment(startDate.date, experiment)) {
        return invalidSnapshot("The planned assignment date is outside the experiment's current planned date range.");
      }

      const animals = await tx.animal.findMany({
        where: { animalId: { in: assignments.map((assignment) => assignment.animalId) } },
        select: {
          id: true,
          animalId: true,
          owningLabId: true,
      ...
      if (animals.length !== assignments.length) {
        return invalidSnapshot("One or more animals in the rendered cohort no longer exist.");
      }
      const animalsByCode = new Map(animals.map((animal) => [animal.animalId, animal]));
      for (const snapshot of assignments) {
        ...
        if (!animal || animal.owningLabId !== experiment.labId) {
          return invalidSnapshot("Every cohort animal must still belong to the experiment lab.");
        }
        if (animal.outcomeStatus !== "alive" || !planningAnimalStatuses.includes(animal.status as (typeof planningAnimalStatuses)[number])) {
          return invalidSnapshot(`${animal.animalId} is no longer live and eligible for experiment planning.`);
        }
        if (animal.experimentAssignments.length > 0) {
          return invalidSnapshot(`${animal.animalId} is already linked to this experiment.`);
        }
      }
      ...
      if (created.count !== assignmentRows.length) {
        await tx.experimentAssignment.deleteMany({ where: { id: { in: assignmentRows.map((row) => row.id) } } });
        return { ok: false, code: "duplicate_assignment", message: "At least one animal is already linked to this experiment. Refresh the assignment worksheet." };
      }
      if (!await bumpExperimentVersion(tx, experiment.id, input.expectedExperimentVersion)) {
        await tx.experimentAssignment.deleteMany({ where: { id: { in: assignmentRows.map((row) => row.id) } } });
        const current = await tx.experiment.findUnique({ where: { id: experiment.id }, select: { version: true } });
        return staleConflict("experiment", experiment.id, input.expectedExperimentVersion, current?.version ?? null);
      }

      const timestamp = new Date();
      for (const row of assignmentRows) {
        const snapshot = assignments.find((assignment) => animalsByCode.get(assignment.animalId)?.id === row.animalId)!;
        await writeAudit(tx, {
          actorId: input.actor.id,
          entityType: "experiment_assignment",
          entityId: row.id,
      ...
      return {
        ok: true,
        result: { experimentId, experimentVersion, assignmentIds: assignmentRows.map((row) => row.id), message },
...
function normalizeAssignmentSnapshot(assignments: ExperimentAssignmentVersionSnapshot[]) {
...
function validateAssignmentSnapshot(assignments: ExperimentAssignmentVersionSnapshot[]) {
  return assignments.length >= 1
    && assignments.length <= 100
    && assignments.every((assignment) => assignment.assignmentId && isPositiveVersion(assignment.expectedVersion))
    && hasUniqueValues(assignments.map((assignment) => assignment.assignmentId));
}

export async function executePromoteExperimentAssignmentsCommand(input: {
  actor: ResolvedActor;
  command: {
    experimentId: string;
  ...
  if (!hasValidIdentity(input) || !experimentId || !isPositiveVersion(input.expectedExperimentVersion) || !validateAssignmentSnapshot(assignments)) {
    return validationError("Submit the exact planned assignment snapshot and refresh all versions before promotion.");
  }
  ...
  return executeIdempotentCommand<AssignmentCommandResult>({
    ...
    handler: async (tx, context) => {
      ...
      if (!experiment || !mutableExperimentStatuses.includes(experiment.status)) {
        return { ok: false, code: "not_found", message: "The experiment was not found or no longer accepts cohort promotion." };
      }
      if (input.actor.canonicalRole === "lab_user" && experiment.labId !== input.actor.activeLabId) {
        return { ok: false, code: "forbidden", message: "The experiment is outside your active lab." };
      }
      ...
      const records = await tx.experimentAssignment.findMany({
        where: { id: { in: assignments.map((assignment) => assignment.assignmentId) } },
        select: {
          id: true,
          experimentId: true,
          status: true,
      ...
      if (records.length !== assignments.length) {
        return invalidSnapshot("One or more planned assignments no longer exist.");
      }
      const expectedById = new Map(assignments.map((assignment) => [assignment.assignmentId, assignment.expectedVersion]));
      for (const assignment of records) {
        const expectedVersion = expectedById.get(assignment.id)!;
        if (assignment.version !== expectedVersion) return assignmentStaleConflict(assignment.id, expectedVersion, assignment.version);
        if (assignment.experimentId !== experiment.id || assignment.status !== "planned") {
          return invalidSnapshot("Every submitted assignment must still be planned for the selected experiment.");
        }
        if (!assignmentDatesAreValid(assignment) || !dateFallsWithinExperiment(assignment.startDate, experiment)) {
          return invalidSnapshot("An assignment date is invalid or outside the experiment's current planned range.");
        }
        if (
          ...
          return invalidSnapshot(`${assignment.animal.animalId} is no longer available for reservation.`);
        }
        if (!assignment.animal.alleles.length || assignment.animal.alleles.some((allele) => allele.callStatus !== "confirmed")) {
          return invalidSnapshot(`${assignment.animal.animalId} requires confirmed genotype calls before promotion.`);
        }
      }
      ...
      for (const assignment of records) {
        ...
        const openAllocation = await tx.animalProjectAllocation.findFirst({
          where: { animalId: assignment.animal.id, projectId: experiment.projectId, endedAt: null },
          select: { id: true },
        });
        let createdAllocationId: string | null = null;
        if (!openAllocation) {
        ...
        await writeAudit(tx, {
          ...
          action: "promote_plan",
          previousValue: { status: "planned", version: assignment.version, animalStatus: previousAnimalStatus },
          newValue: {
            receiptId: context.receiptId,
            status: "reserved",
            ...
            autoAllocationId: createdAllocationId,
            autoAllocationMarker: createdAllocationId ? experimentPromotionAllocationMarker(assignment.id) : null,
          },
        });
      }
      const experimentVersion = input.expectedExperimentVersion + 1;
      await writeAudit(tx, {
        ...
        previousValue: { version: input.expectedExperimentVersion },
        newValue: { receiptId: context.receiptId, version: experimentVersion, assignments },
      });
      const message = `Promoted ${records.length} planned assignment${records.length === 1 ? "" : "s"} for ${experiment.experimentCode}.`;
      return {
        ok: true,
        result: { experimentId, experimentVersion, assignmentIds: assignments.map((assignment) => assignment.assignmentId), message },
        resultingVersion: experimentVersion,
      };
    },
  });
...
export async function executeDemoteExperimentAssignmentsCommand(input: {
  actor: ResolvedActor;
  command: {
    experimentId: string;
    assignments: ExperimentAssignmentVersionSnapshot[];
  ...
  const assignments = normalizeAssignmentSnapshot(input.command.assignments);
  if (!hasValidIdentity(input) || !experimentId || !isPositiveVersion(input.expectedExperimentVersion) || !validateAssignmentSnapshot(assignments)) {
    return validationError("Submit the exact reserved assignment snapshot and refresh all versions before rollback.");
  }
  const request = { experimentId, assignments };
  return executeIdempotentCommand<AssignmentCommandResult>({
    actor: input.actor,
    ...
    handler: async (tx, context) => {
      ...
      if (!experiment || !mutableExperimentStatuses.includes(experiment.status)) {
        return { ok: false, code: "not_found", message: "The experiment was not found or no longer accepts cohort rollback." };
      }
      if (input.actor.canonicalRole === "lab_user" && experiment.labId !== input.actor.activeLabId) {
        return { ok: false, code: "forbidden", message: "The experiment is outside your active lab." };
      }
      ...
      const [records, promotionAudits] = await Promise.all([
        ...
        tx.auditLog.findMany({
          where: { entityType: "experiment_assignment", entityId: { in: assignmentIds }, action: "promote_plan" },
          select: { entityId: true },
        }),
      ]);
      if (records.length !== assignments.length) return invalidSnapshot("One or more reserved assignments no longer exist.");
      const promotedIds = new Set(promotionAudits.map((audit) => audit.entityId));
      const expectedById = new Map(assignments.map((assignment) => [assignment.assignmentId, assignment.expectedVersion]));
      for (const assignment of records) {
        const expectedVersion = expectedById.get(assignment.id)!;
        if (assignment.version !== expectedVersion) return assignmentStaleConflict(assignment.id, expectedVersion, assignment.version);
        if (assignment.experimentId !== experiment.id || assignment.status !== "reserved" || !promotedIds.has(assignment.id)) {
          return invalidSnapshot("Every submitted assignment must still be a reservation created by cohort promotion.");
        }
        if (!assignmentDatesAreValid(assignment)) return invalidSnapshot("An assignment contains an invalid date range.");
        if (
          assignment.animal.owningLabId !== experiment.labId
          || assignment.animal.outcomeStatus !== "alive"
          || assignment.animal.status !== "reserved"
        ) {
          return invalidSnapshot(`${assignment.animal.animalId} no longer has rollback-compatible reservation state.`);
        }
      }
      ...
      const timestamp = new Date();
      for (const assignment of records) {
        const remainingReservations = assignment.animal.experimentAssignments.filter((linked) => !snapshotIdSet.has(linked.id));
        const targetAnimalStatus = remainingReservations.length ? "reserved" as const : "colony_holding" as const;
        await tx.experimentAssignment.update({
          where: { id: assignment.id },
        ...
        await tx.animal.update({
          ...
          data: {
            ...
            projectSummary: remainingReservations[0]?.experiment.project.projectCode ?? null,
            version: { increment: 1 },
          },
        });

        const hasOtherOpenProjectNeed = remainingReservations.some((linked) => linked.experiment.projectId === experiment.projectId);
        ...
        if (!hasOtherOpenProjectNeed) {
          const ended = await tx.animalProjectAllocation.updateMany({
            where: {
              ...
              projectId: experiment.projectId,
              endedAt: null,
              notes: { contains: experimentPromotionAllocationMarker(assignment.id) },
            },
...
export async function executeUpdatePlannedExperimentAssignmentCommand(input: {
  actor: ResolvedActor;
  command: {
    experimentId: string;
    assignmentId: string;
  ...
  if (
    ...
    return validationError("Submit valid planned-assignment values and refresh both record versions.");
  }
  const request = { experimentId, assignmentId, startDate: startDate.serialized, treatmentGroup, notes, expectedAssignmentVersion: input.expectedAssignmentVersion };
  return executeIdempotentCommand<AssignmentCommandResult>({
    actor: input.actor,
    ...
    handler: async (tx, context) => {
      ...
      if (!assignment || assignment.experimentId !== experimentId || assignment.status !== "planned") {
        return { ok: false, code: "not_found", message: "The planned assignment was not found." };
      }
      if (input.actor.canonicalRole === "lab_user" && assignment.experiment.labId !== input.actor.activeLabId) {
        return { ok: false, code: "forbidden", message: "The assignment is outside your active lab." };
      }
      if (
        assignment.animal.owningLabId !== assignment.experiment.labId
        || !mutableExperimentStatuses.includes(assignment.experiment.status)
      ) {
        return invalidSnapshot("The assignment is no longer editable in its experiment lab.");
      }
      if (assignment.experiment.version !== input.expectedExperimentVersion) {
        return staleConflict("experiment", experimentId, input.expectedExperimentVersion, assignment.experiment.version);
      }
      if (assignment.version !== input.expectedAssignmentVersion) {
        return assignmentStaleConflict(assignment.id, input.expectedAssignmentVersion, assignment.version);
      }
      if (!dateFallsWithinExperiment(startDate.date, assignment.experiment)) {
        return invalidSnapshot("The assignment date is outside the experiment's current planned range.");
      }
      if (!await bumpExperimentVersion(tx, experimentId, input.expectedExperimentVersion)) {
        const current = await tx.experiment.findUnique({ where: { id: experimentId }, select: { version: true } });
        return staleConflict("experiment", experimentId, input.expectedExperimentVersion, current?.version ?? null);
      }
      await tx.experimentAssignment.update({
        where: { id: assignment.id },
        data: { startDate: startDate.date, treatmentGroup, notes, version: { increment: 1 } },
      });
      await writeAudit(tx, {
        actorId: input.actor.id,
        entityType: "experiment_assignment",
      ...
      await writeAudit(tx, {
        ...
        previousValue: { version: input.expectedExperimentVersion },
        newValue: { receiptId: context.receiptId, version: experimentVersion, assignmentId, assignmentVersion: assignment.version + 1 },
      });
      const message = `Updated planned assignment for ${assignment.animal.animalId} in ${assignment.experiment.experimentCode}.`;
      return {
        ok: true,
        result: { experimentId, experimentVersion, assignmentIds: [assignmentId], message },
        resultingVersion: experimentVersion,
      };
    },
  });
...
export async function executeDeletePlannedExperimentAssignmentCommand(input: {
  actor: ResolvedActor;
  ...
  if (
    ...
    return validationError("Refresh both record versions before removing this planned assignment.");
  }
  const request = { experimentId, assignmentId, expectedAssignmentVersion: input.expectedAssignmentVersion };
  return executeIdempotentCommand<AssignmentCommandResult>({
    actor: input.actor,
    ...
    handler: async (tx, context) => {
      const assignment = await tx.experimentAssignment.findUnique({
        ...
        select: {
          ...
          animal: { select: { animalId: true, owningLabId: true } },
          experiment: {
            select: { id: true, labId: true, experimentCode: true, status: true, version: true },
          },
        },
      });
      if (!assignment || assignment.experimentId !== experimentId || assignment.status !== "planned") {
      ...
      if (
        ...
        return invalidSnapshot("The assignment is no longer removable in its experiment lab.");
      }
      if (assignment.experiment.version !== input.expectedExperimentVersion) {
        return staleConflict("experiment", experimentId, input.expectedExperimentVersion, assignment.experiment.version);
      }
      ...
      await writeAudit(tx, {
        ...
        previousValue: { version: input.expectedExperimentVersion },
        newValue: { receiptId: context.receiptId, version: experimentVersion, assignmentId, deletedAssignmentVersion: assignment.version },
      });
