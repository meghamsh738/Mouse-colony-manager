...
import { resolveEffectiveCageCapacity, validateCapacityOverride } from "@/lib/cage-capacity";
import {
...
import { canManageLab, getActorLabAccess, type LabActor } from "@/lib/lab-access";
...
function destinationKey(destination: CageDestinationRef) {
  return destination.kind === "new" ? `new:${destination.clientId}` : `existing:${destination.cageId}`;
}
...
async function prepareNewCages(
...
): Promise<{ ok: true; cages: PreparedCage[] } | { ok: false; message: string }> {
  const access = await getActorLabAccess(actor, tx);
  const clientIds = drafts.map((draft) => draft.clientId.trim());

  if (new Set(clientIds).size !== clientIds.length || clientIds.some((id) => !id)) {
    return { ok: false, message: "Each proposed cage needs a unique planning ID." };
  }
  ...
  const [rooms, racks, activeCategories] = await Promise.all([
    ...
    tx.cageChargeCategory.findMany({
      ...
      select: { id: true, code: true, dailyRateCents: true, currencyCode: true },
    }),
  ]);
  const defaultCategory = activeCategories.find((category) => category.code === "STANDARD") ?? activeCategories[0] ?? null;
  const roomById = new Map(rooms.map((room) => [room.id, room]));
  const rackById = new Map(racks.map((rack) => [rack.id, rack]));
  const requestedCategoryIds = Array.from(
    new Set(drafts.map((draft) => draft.chargeCategoryId).filter((id): id is string => Boolean(id))),
  );
  ...
  for (const draft of drafts) {
    ...
    if (!room || !rack || rack.roomId !== room.id) {
      return { ok: false, message: "Choose a rack that belongs to the selected room." };
    }

    if (!canManageLab(access, draft.labId)) {
      return { ok: false, message: "You can only create cages for labs you manage." };
    }

    if (!startDate.ok) return startDate;

    const cageNumber = draft.cageNumber.trim();
    if (!cageNumber) return { ok: false, message: "Enter a cage number for every proposed cage." };
    ...
    const category = draft.chargeCategoryId ? categoryById.get(draft.chargeCategoryId) : defaultCategory;

    if (!barcode) return { ok: false, message: "Enter or generate a cage barcode." };
    if (!category) return { ok: false, message: "Configure an active cage charge category before creating cages." };
  ...
  if (new Set(prepared.map((cage) => `${cage.rackId}:${cage.cageNumber}`)).size !== prepared.length) {
    return { ok: false, message: "A cage number can only be used once on the same rack." };
  }
  ...
  const conflict = await tx.cage.findFirst({
    where: {
      OR: [
        ...
        ...prepared.map((cage) => ({ rackId: cage.rackId, cageNumber: cage.cageNumber })),
      ],
    },
    select: { barcode: true },
  });
  ...
  if (conflict) {
    return { ok: false, message: `Cage ID or rack position already exists (${conflict.barcode}).` };
  }

  if (!prepared.length) return { ok: true, cages: [] };
  const facilityCageIds = await allocateFacilityIdentifiers(tx, "cage", prepared.length);
  return {
    ok: true,
    cages: prepared.map((cage, index) => ({ ...cage, facilityCageId: facilityCageIds[index] })),
  };
}
...
async function prepareExistingDestinations(
  ...
  actor: LabActor,
): Promise<{ ok: true; cages: Map<string, PreparedDestination> } | { ok: false; message: string }> {
  const access = await getActorLabAccess(actor, tx);
  ...
  if (cages.length !== new Set(destinationIds).size) {
    return { ok: false, message: "One or more destination cages could not be found." };
  }

  const result = new Map<string, PreparedDestination>();
  for (const cage of cages) {
    if (!cage.labId || !canManageLab(access, cage.labId)) {
      return { ok: false, message: `You cannot assign animals to ${cage.barcode}.` };
    }
    if (!cage.active || cage.status === "closed" || cage.status === "retired") {
      return { ok: false, message: `${cage.barcode} is not an active operational cage.` };
    }
...
function validateDestinationCapacities(
  ...
  for (const cage of existingCages.values()) {
    ...
    if (projected > cage.effectiveCapacity) {
      const available = Math.max(0, cage.effectiveCapacity - cage.occupantCount + outgoing);
...
function validateDestinationReferences(
  ...
  for (const destination of destinations) {
    ...
    if (destination.kind === "existing" && !existingCages.has(destination.cageId)) {
...
export async function createCageWithAssignments(
  ...
  if (!canManageCageIntake(actor.role)) {
    return { ok: false, message: "Your role cannot create cages or assign animals." };
  }
  if (!input.cages.length) return { ok: false, message: "Add at least one cage to the plan." };

  const movedAt = parseCageIntakeDate(input.movedAt, "assignment date");
  const reason = input.reason.trim();
  if (!movedAt.ok) return movedAt;
  if (reason.length < 3) return { ok: false, message: "Enter a concise reason for the cage assignment." };
  if (new Set(input.assignments.map((assignment) => assignment.subjectId)).size !== input.assignments.length) {
    return { ok: false, message: "Each animal can only be assigned once in a cage plan." };
  }

  const operation = async (tx: Prisma.TransactionClient): Promise<MutationResult> => {
    const incoming = buildIncomingCounts(input.assignments.map((assignment) => assignment.destination));
    ...
    const existingDestinationIds = input.assignments.flatMap((assignment) =>
      assignment.destination.kind === "existing" ? [assignment.destination.cageId] : [],
    );
    const existingDestinations = await prepareExistingDestinations(tx, existingDestinationIds, actor);
    if (!existingDestinations.ok) return existingDestinations;

    const animals = input.assignments.length
      ? await tx.animal.findMany({
          where: { id: { in: input.assignments.map((assignment) => assignment.subjectId) } },
          select: {
            id: true,
            animalId: true,
            sex: true,
    ...
    if (animals.length !== input.assignments.length) {
      return { ok: false as const, message: "One or more selected animals could not be found." };
    }

    const animalById = new Map(animals.map((animal) => [animal.id, animal]));
    const outgoing = animals.reduce<Map<string, number>>((counts, animal) => {
      if (animal.currentCageId) counts.set(animal.currentCageId, (counts.get(animal.currentCageId) ?? 0) + 1);
      return counts;
    }, new Map());
    const capacityError = validateDestinationCapacities(prepared.cages, existingDestinations.cages, incoming, outgoing);
    if (capacityError) return { ok: false as const, message: capacityError };

    const newCageByClientId = new Map(prepared.cages.map((cage) => [cage.clientId, cage]));
    ...
    if (destinationError) return { ok: false as const, message: destinationError };

    for (const assignment of input.assignments) {
      const animal = animalById.get(assignment.subjectId);
      const destinationId = resolveDestinationId(assignment.destination, newCageByClientId);
      const destination =
      ...
      if (!animal || !destinationId || !destination) {
        return { ok: false as const, message: "Every selected animal needs a valid destination cage." };
      }
      if (animal.outcomeStatus !== "alive" || !animal.currentCageId || !animal.currentCage) {
        return { ok: false as const, message: `${animal.animalId} is not an assignable live colony animal.` };
      }
      if (animal.currentCageId === destinationId) {
        return { ok: false as const, message: `${animal.animalId} is already in ${destination.barcode}.` };
      }
      const sourceLabId = animal.currentCage.labId ?? animal.owningLabId;
      if (sourceLabId && sourceLabId !== destination.labId) {
        return { ok: false as const, message: "Use the cross-lab transfer approval workflow to change animal ownership." };
      }
    }

    const ruleDestinations = assignmentRuleDestinations(prepared.cages, existingDestinations.cages);
    const quarantineError = validateQuarantineAssignments({
      workflow: "new",
      destinations: ruleDestinations,
      usedDestinationKeys: new Set(input.assignments.map((assignment) => destinationKey(assignment.destination))),
    });
    if (quarantineError) return { ok: false as const, message: quarantineError };
    const sexError = validateProjectedSexComposition({
      destinations: ruleDestinations,
      incoming: input.assignments.flatMap((assignment) => {
        const animal = animalById.get(assignment.subjectId);
        return animal
          ? [{ subjectId: animal.id, sex: animal.sex, destinationKey: destinationKey(assignment.destination) }]
          : [];
      }),
    ...
    for (const assignment of input.assignments) {
      ...
      const destination =
        assignment.destination.kind === "new"
          ? newCageByClientId.get(assignment.destination.clientId)
          : existingDestinations.cages.get(assignment.destination.cageId);
      if (!animal || !destinationId || !destination || !animal.currentCageId) continue;
    ...
    await tx.cage.updateMany({
      where: { id: { in: [...prepared.cages.map((cage) => cage.id), ...existingDestinationIds] } },
    ...
    return {
      ...
      message: `${prepared.cages.length} ${prepared.cages.length === 1 ? "cage" : "cages"} created${
        input.assignments.length ? ` and ${input.assignments.length} animals assigned` : ""
      }.`,
...
export async function weanLitterWithCagePlan(
  ...
  if (!input.pups.length) return { ok: false, message: "Add at least one pup to the weaning plan." };
  ...
  const operation = async (tx: Prisma.TransactionClient): Promise<MutationResult> => {
    ...
    const [litter, strain, existingAnimals] = await Promise.all([
          ...
          breedingSetup: {
            select: {
              ...
              adults: {
                select: {
                  role: true,
                  animal: { select: { id: true, projectSummary: true, owningLabId: true } },
                },
              },
            },
          },
      ...
      tx.strain.findUnique({ where: { id: input.strainId }, select: { id: true } }),
      tx.animal.findMany({ select: { animalId: true, labId: true } }),
    ]);
    if (!litter) return { ok: false as const, message: "Litter not found." };
    if (!strain) return { ok: false as const, message: "Choose a valid strain." };
    ...
    if (weanDate.date < litter.birthDate) {
      return { ok: false as const, message: "Weaning date cannot be earlier than the litter birth date." };
    }
    if (input.pups.length > litter.litterSizeBirth) {
      return { ok: false as const, message: "Weaning count cannot exceed litter size at birth." };
    }
    ...
    if (
      ...
      return { ok: false as const, message: "You cannot wean a litter owned by another lab." };
    }

    const incoming = buildIncomingCounts(input.pups.map((pup) => pup.destination));
    const prepared = await prepareNewCages(
      tx,
      input.cages.map((cage) => ({ ...cage, startDate: input.weanDate, status: "active" })),
      actor,
      incoming,
    );
    if (!prepared.ok) return prepared;
    ...
    const existingDestinations = await prepareExistingDestinations(tx, existingIds, actor);
    if (!existingDestinations.ok) return existingDestinations;
    const destinationError = validateDestinationReferences(
    ...
    if ([...destinationLabIds].some((labId) => labId !== breedingLabId)) {
      return { ok: false as const, message: "Wean progeny into cages owned by the breeding setup's lab." };
    }
    const capacityError = validateDestinationCapacities(prepared.cages, existingDestinations.cages, incoming);
    if (capacityError) return { ok: false as const, message: capacityError };
    ...
    const quarantineError = validateQuarantineAssignments({
      workflow: "wean",
      destinations: ruleDestinations,
      usedDestinationKeys: new Set(input.pups.map((pup) => destinationKey(pup.destination))),
    });
    if (quarantineError) return { ok: false as const, message: quarantineError };
    const sexError = validateProjectedSexComposition({
      destinations: ruleDestinations,
    ...
    if (destinations.some((destination) => !destination.cageId)) {
      return { ok: false as const, message: "Every pup must have a valid destination cage." };
    }
    ...
    const projectSummary = projectSummaries.size === 1 ? [...projectSummaries][0] : null;
    const identifiers = buildNextAnimalIdentifiers(existingAnimals, litter.birthDate.getUTCFullYear(), input.pups.length);
    const facilityAnimalIds = await allocateFacilityIdentifiers(tx, "animal", input.pups.length);
    ...
    for (const animal of animals) {
      ...
      await tx.litterAnimal.create({
        data: { id: createId("litter-animal"), litterId: litter.id, animalId: animal.id },
      });
    ...
    await tx.litter.update({ where: { id: litter.id }, data: { litterSizeWean: animals.length } });
    await tx.cage.updateMany({
      where: { id: { in: Array.from(new Set(destinations.map((destination) => destination.cageId!))) } },
    ...
    await tx.auditLog.create({
      data: {
        ...
        entityId: litter.id,
        action: "wean",
        previousValue: { litterSizeWean: litter.litterSizeWean, progenyCount: litter._count.litterAnimals },
        newValue: {
...
export async function receivePurchasedAnimals(
  ...
  if (!canManageCageIntake(actor.role)) {
    return { ok: false, message: "Your role cannot receive purchased animals." };
  }
  const vendor = input.vendor.trim();
  const orderReference = input.orderReference.trim();
  const arrivalDate = parseCageIntakeDate(input.arrivalDate, "arrival date");
  if (vendor.length < 2) return { ok: false, message: "Enter the vendor or source." };
  if (orderReference.length < 2) return { ok: false, message: "Enter the order or receipt reference." };
  if (!arrivalDate.ok) return arrivalDate;
  if (!input.animals.length) return { ok: false, message: "Add at least one purchased animal." };
  if (new Set(input.animals.map((animal) => animal.rowId)).size !== input.animals.length) {
    return { ok: false, message: "Each purchased animal row must be unique." };
  }
  const sourceIds = input.animals.map((animal) => animal.sourceAnimalId?.trim()).filter((id): id is string => Boolean(id));
  if (new Set(sourceIds).size !== sourceIds.length) {
    return { ok: false, message: "Vendor animal IDs must be unique within a delivery." };
  }

  const operation = async (tx: Prisma.TransactionClient): Promise<MutationResult> => {
    const access = await getActorLabAccess(actor, tx);
    if (!canManageLab(access, input.labId)) {
      return { ok: false, message: "You can only receive animals for labs you manage." };
    }
    const lab = await tx.lab.findUnique({ where: { id: input.labId }, select: { id: true, active: true } });
    if (!lab?.active) return { ok: false as const, message: "Choose an active owning lab." };

    const incoming = buildIncomingCounts(input.animals.map((animal) => animal.destination));
    if (input.cages.some((cage) => cage.labId !== input.labId)) {
      return { ok: false as const, message: "Purchased intake cages must belong to the receiving lab." };
    }
    const prepared = await prepareNewCages(
      tx,
    ...
    if (destinationError) return { ok: false as const, message: destinationError };
    if ([...existingDestinations.cages.values()].some((cage) => cage.labId !== input.labId)) {
      return { ok: false as const, message: "Purchased animals must be assigned to cages in the receiving lab." };
    }
    const capacityError = validateDestinationCapacities(prepared.cages, existingDestinations.cages, incoming);
    if (capacityError) return { ok: false as const, message: capacityError };
    ...
    const quarantineError = validateQuarantineAssignments({
      ...
      destinations: ruleDestinations,
      usedDestinationKeys: new Set(input.animals.map((animal) => destinationKey(animal.destination))),
    });
    if (quarantineError) return { ok: false as const, message: quarantineError };
    const sexError = validateProjectedSexComposition({
      destinations: ruleDestinations,
    ...
    const strainIds = Array.from(new Set(input.animals.map((animal) => animal.strainId)));
    const strains = await tx.strain.findMany({ where: { id: { in: strainIds } }, select: { id: true } });
    if (strains.length !== strainIds.length) return { ok: false as const, message: "Choose a valid strain for every row." };
    for (const row of input.animals) {
      const dob = parseCageIntakeDate(row.dob, "date of birth");
      if (!dob.ok) return { ok: false as const, message: `Row ${row.rowId}: ${dob.message}` };
      if (dob.date > arrivalDate.date) {
        return { ok: false as const, message: `Row ${row.rowId}: date of birth cannot be after arrival.` };
      }
    }

    const existingAnimals = await tx.animal.findMany({ select: { animalId: true, labId: true } });
    const identifiers = buildNextAnimalIdentifiers(existingAnimals, arrivalDate.date.getUTCFullYear(), input.animals.length);
    const facilityAnimalIds = await allocateFacilityIdentifiers(tx, "animal", input.animals.length);
    const batchId = createId("intake");
    const newCageByClientId = new Map(prepared.cages.map((cage) => [cage.clientId, cage]));
    await createPreparedCages(tx, prepared.cages, actor.id);
    ...
    for (const [index, row] of input.animals.entries()) {
      ...
      const destinationId = resolveDestinationId(row.destination, newCageByClientId);
      ...
      await tx.animal.create({
        data: {
          ...
          healthStatus: input.disposition === "quarantine" ? "Quarantine intake" : "Healthy",
...
function commandLabId(command: CageIntakeCommand, actor: ResolvedActor) {
  ...
  const proposedLabs = [...new Set(command.payload.cages.map((cage) => cage.labId).filter(Boolean))];
...
export async function executeCageIntakeCommand(input: {
  ...
  return executeIdempotentCommand({
    ...
    aggregateType: input.command.mode === "wean" ? "litter" : undefined,
    aggregateId: input.command.mode === "wean" ? input.command.payload.litterId : undefined,
    expectedVersion: input.command.mode === "wean" ? input.expectedVersion : undefined,
    handler: async (tx) => {
      if (!input.workflowDraftId || !input.reviewSnapshotId) {
        return { ok: false as const, code: "invalid_review", message: "Create an intake review snapshot before confirming." };
      }
      const snapshot = await tx.workflowReviewSnapshot.findUnique({
        where: { id: input.reviewSnapshotId },
        include: { draft: true },
      });
      const reviewedPayload = snapshot ? cageIntakeDraftPayloadSchema.safeParse(snapshot.payload) : null;
      if (
        !snapshot
        ...
        || canonicalJsonHash(reviewedPayload.data.command) !== canonicalJsonHash(input.command)
      ) {
        return { ok: false as const, code: "invalid_review", message: "The reviewed intake changed. Refresh and review it again." };
      }
      ...
      if (reserved.count !== 1) {
        return { ok: false as const, code: "stale_conflict", message: "The intake review changed before it could be submitted." };
      }
      ...
      const result = input.command.mode === "new"
        ? await createCageWithAssignments(input.command.payload, operationalActor, tx)
        : input.command.mode === "wean"
          ? await weanLitterWithCagePlan(input.command.payload, operationalActor, tx)
          : await receivePurchasedAnimals(input.command.payload, operationalActor, tx);
      if (!result.ok) {
        ...
        if (restored.count !== 1) throw new Error("The failed intake review could not be restored atomically.");
      ...
      if (committed.count !== 1) throw new Error("The reviewed intake could not be committed atomically.");
      return {
        ok: true as const,
        result: {
        ...
        aggregateType: input.command.mode === "wean" ? "litter" : input.command.mode === "purchase" ? "intake_batch" : "cage_intake",
