...
export default async function SamplesPage({ searchParams }: SamplesPageProps) {
  ...
  const requestedAction = getRequestedAction(rawQuery, ["add-sample"] as const);
  const shouldLoadCreateOptions = canRecordSample && requestedAction === "add-sample";
  const [inventory, options, filterOptions] = await Promise.all([
    getSampleInventoryPageView(user, rawQuery),
    shouldLoadCreateOptions ? getSamplePageOptions(user) : Promise.resolve(null),
    shouldLoadCreateOptions ? Promise.resolve(null) : getSampleInventoryFilterOptions(user),
  ]);
  ...
  const showRecordSampleForm = canRecordSample && options !== null;
  ...
  const inventoryHref = canonicalSamplesHref(inventory.query, inventory.page);
    ...
    ? [
        {
          ...
          ...(showRecordSampleForm
            ...
            : { href: withActionQuery(inventoryHref, "add-sample") }),
        },
      ]
    : [];
    ...
    <AppShell currentPath="/samples" role={user.role} userName={user.name ?? user.email ?? "Unknown user"}>
      <div className="space-y-5">
        ...
        {canRecordSample ? (
          <CompactActionTray
            actions={actions}
            closeHref={showRecordSampleForm ? inventoryHref : undefined}
            defaultActionId={showRecordSampleForm ? "add-sample" : undefined}
            eyebrow="Actions"
