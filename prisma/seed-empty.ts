...
async function seedEmptyDatabase() {
  ...
  await prisma.$transaction(async (tx) => {
    const operationalCounts = {
      ...
      correctionRequests: await tx.correctionRequest.count(),
      correctionEvents: await tx.correctionLifecycleEvent.count(),
      correctionSupersessions: await tx.correctionSupersession.count(),
      correctionReconciliations: await tx.correctionReconciliation.count(),
