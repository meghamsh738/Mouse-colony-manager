...
- Done for MVP-level runtime use:
  ...
  - authenticated `/api/v1` integration routes for animals, cages, experiments, projects, samples, genotype intake, and export discovery, including audited external sample and genotype result intake
...
- Still missing relative to the original blueprint:
  - broader external integration API beyond current sample/genotype intake, read-mostly `/api/v1`, CSV export endpoints, and notification delivery preview/webhook surface
...
- [ ] Define the next external write integration after sample and genotype intake, likely experiment assignment sync or external equipment event ingestion
