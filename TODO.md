...
- Done for MVP-level runtime use:
  ...
  - authenticated `/api/v1` integration routes for animals, cages, breeding setup intake, experiments, projects, samples, genotype intake, experiment assignment sync, cage welfare event intake, and export discovery, including audited external sample intake, sample lifecycle updates, genotype result intake, breeding setup intake, planned-assignment intake, assignment promote/rollback status sync, and cage health-note ingestion
...
- [ ] Define the next external write integration after breeding setup intake, animal lifecycle, cage moves, sample, cryostorage, genotype, attachment-aware cage welfare intake, and experiment assignment sync/status
